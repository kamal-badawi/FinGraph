# Wichtiger Hinweis:
# Die Optimierung des Forecasting-Modells (Facebook Prophet) wurde durch ChatGpt durchgeführt
def run_forecasting(language_index, app_title):
    ...
    def create_forecast_line_chart(line_chart_data, is_forecast_line_chart_date_from, is_forecast_line_chart_date_to,
        ...
        fig.update_layout(
            ...
            title=dict(
                text=f' Forecast zwischen {is_forecast_line_chart_date_from} und {is_forecast_line_chart_date_to} für {is_forecast_line_chart_ticker}',
                x=0.5,
                xanchor='center',
                font=dict(size=25)
            )
    ...
    def forecasting_get_data(asset_type,
                             selected_ticker,
                             training_date_from,
                             training_date_to,
                             forecast_start_date,
    ...
    forecasting_asset_type_col, forecasting_selected_ticker_col = st.sidebar.columns(2)
    with forecasting_asset_type_col:
        forecasting_asset_type_options = ['Aktien', 'Kryptowährungen', 'Währungen', 'Rohstoffe', 'Fonds']
        forecasting_asset_type = st.selectbox(label='Asset-Typ:',
    ...
    with training_date_from_col:
        training_date_from = st.date_input(label='Von-Datum (Training):',
    ...
    with training_date_to_col:
        training_date_to = st.date_input(label='Bis-Datum (Training):',
                                         value=today,
                                         min_value=today,
                                         max_value=today,
                                         disabled=True,
    ...
    with selected_interval_col:
        ...
        selected_forecast_interval = st.selectbox("Forecast-Intervall:",
                                                  options=intervals,
                                                  index=1,
    ...
    with count_of_periods_col:
        count_of_forecast_periods = st.number_input(label='Anz. d. Perioden:',
                                                    min_value=1,
                                                    max_value=1000,
                                                    value=12,
    ...
    forecast_data,  title = forecasting_get_data(forecasting_asset_type,
                                                             selected_ticker,
                                                             training_date_from,
                                                             training_date_to,
                                                             forecast_start_date,
